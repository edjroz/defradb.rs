//! Peeling is probabilistic, so convergence is property-tested rather than
//! exemplified: for arbitrary set pairs, a session must terminate with the
//! decoder's need and have sets *exactly* equal to the true symmetric
//! difference — no misses, no spurious entries, and no run that gives up.
//!
...
//!
//! An unlucky prefix is not a failure mode here — the decoder answers a stall by
//! reading more symbols, up to [`caps::MAX_CODED_SYMBOLS`]. What these tests
//! would catch is a run that reaches that cap, which is what a genuinely broken
//! mapping or peel looks like from outside.
//!
//! [`caps::MAX_CODED_SYMBOLS`]: super::caps::MAX_CODED_SYMBOLS
...
proptest! {
    ...
    /// Sets far past a single batch, so every session exercises the stall-and-
    /// read-more path many times over.
    #[test]
...
fn an_empty_set_against_a_full_one_yields_the_whole_set() {
    let seeds: Vec<u64> = (0..500).collect();
    check(&[], &seeds).expect("the decoder needs everything");
    check(&seeds, &[]).expect("the decoder has everything");
}
